...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.addEventListener("load", () => {
        if (promoPopup) promoPopup.style.display = "block";
        if (promoOverlay) promoOverlay.style.display = "block";
    });
    ...
    if (promoOverlay) promoOverlay.addEventListener("click", handleClosePopup);
});
...
async function cargarCategorias() {
  ...
  `;
        ...
        if (typeof inicializarCategorias === 'function') inicializarCategorias();
...
function abrirProducto(id) {
    ...
    if (producto) {
        // Lógica para etiquetas sanitarias personalizadas por ID
        // Agrega aquí más IDs separados por comas si es necesario (ej: [13, 17, 99])
...
function initStatsAnimation() {
    ...
    if (isVisible) {
        ...
        animateValue("stat4", 0, 10, 2000, {
            isFraction: true
        });

        // 70% de clientes vuelven
        animateValue("stat5", 0, 70, 2000, {
            isPercentage: true
        });
...
function buscarProductos() {
    ...
    if (searchTerm === "") {
        // Si está vacío, restaurar vista normal. CORREGIDO: Usamos currentCategoryId = null
        currentCategoryId = null; // Antes decía currentCategory = "todos", lo cual causaba error.
        renderizarProductos();
        searchResults.style.display = "none";
        return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function createParticlesFromSVG() {
        ...
        if (!path) return; // Validación extra
